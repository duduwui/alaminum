...
export const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = cardInitialTopsRef.current[i] ?? getElementOffset(card);
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = cardInitialTopsRef.current[j] ?? getElementOffset(cardsRef.current[j]);
      ...
      const safeTranslateY = Math.min(Math.max(translateY, -3000), 3000);

      const newTransform = {
        translateY: Math.round(safeTranslateY * 100) / 100,
    ...
    const recalibrateCardTops = () => {
      ...
      cardInitialTopsRef.current = cards.map((card) => {
        if (useWindowScroll) {
          ...
          const targetEl = wrapper && wrapper.classList.contains('scroll-stack-card-wrapper') ? wrapper : card;
